...
from ozone_alpha.glms.methods.runge_kutta.explicit_runge_kutta import \
...
from ozone_alpha.glms.methods.runge_kutta.implicit_runge_kutta import \
    BackwardEuler, ImplicitMidpoint, TrapezoidalRule
from ozone_alpha.glms.methods.runge_kutta.gauss_legendre import GaussLegendre
from ozone_alpha.glms.methods.runge_kutta.lobatto import LobattoIIIA
from ozone_alpha.glms.methods.runge_kutta.radau import Radau
# from ozone_alpha.glms.methods.linear_multistep.adams import AB, AM
# from ozone_alpha.glms.methods.linear_multistep.adams_alt import ABalt, AMalt
# from ozone_alpha.glms.methods.linear_multistep.bdf import BDF
# from ozone_alpha.glms.methods.linear_multistep.predictor_corrector import AdamsPEC, AdamsPECE

# from original Ozone

class GLMMethod(object):

    def __init__(self, A, B, U, V, explicit):
        self.A:np.ndarray = A
        self.B:np.ndarray = B
        self.U:np.ndarray = U
        self.V:np.ndarray = V
        self.explicit:bool = explicit
        self.num_stages:int = len(self.A)
        self.C:np.ndarray = self.A.dot(np.ones((self.num_stages, 1)))
...
def get_integration_method(string):
    ...
    # print(A)
    return GLMMethod(A, B, U, V, explicit)

...
import numpy as np
from ozone.tests.test_3_large.run_ODE_systems import ODESystemNative, ODESystemCSDL
...
def run_ode(settings_dict):
    ...
    class RunModel(csdl.Model):
        ...
        def define(self):
            ...
            self.create_input('y_0', 0.5*np.ones((STATE_SIZE,)))
            self.create_input('static_param', np.linspace(0.1, 1.1, STATE_SIZE))
            self.create_input('dynamic_param', np.linspace(0.5, 0.8, num_times).reshape((num_times,1)))
            ...
            ode_problem.add_state('y', 'dy_dt', initial_condition_name='y_0', output='y_integrated', shape = (STATE_SIZE,))
            ode_problem.add_parameter('static_param', shape = (STATE_SIZE,))
            ode_problem.add_parameter('dynamic_param', shape = (ode_problem.num_times,1), dynamic=1)
            ...
            y_out = self.declare_variable('y_integrated', shape=(nt,STATE_SIZE))
    ...
    # derivative_checks = sim.check_totals(of=['y_out'], wrt=['y_0', 'h', 'static_param','dynamic_param'])
    derivative_checks = sim.compute_totals(of=['y_out'], wrt=['y_0', 'h', 'static_param','dynamic_param'])

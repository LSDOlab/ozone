...
from GroupSystem import ODEGroup
...
ODEProblemInstance = ODEProblemTest(
    'RK4', 'time-marching', num_times=num, display='visual')
comp = ODEProblemInstance.create_solver_model()
...
t1 = time.time()
prob.run_model()
ODEProblemInstance.check_partials(['ODE'])
prob.check_totals(of=['field_output'], wrt=['A', 'y0', 'h'])
print('run)model time: ', time.time() - t1)
plt.show()
